...
describe('parseToon', () => {
  ...
  it('should throw JSONParseError for invalid TOON', async () => {
    await expect(() => parseToon({ text: 'invalid toon }{' })).rejects.toThrow(
      JSONParseError,
    );
  });
...
describe('safeParseToon', () => {
  ...
  it('should handle failed parsing with error details', async () => {
    const result = await safeParseToon({ text: 'invalid toon }{' });
...
describe('isParsableToon', () => {
  it('should return true for valid TOON', () => {
    expect(isParsableToon(`foo\n  "bar"`)).toBe(true);
    expect(isParsableToon(`items[2]:\n  1\n  2`)).toBe(true);
    expect(isParsableToon(`name\n  "hello"`)).toBe(true);
  });

  it('should return false for invalid TOON', () => {
    expect(isParsableToon('invalid }{')).toBe(false);
  });
});
